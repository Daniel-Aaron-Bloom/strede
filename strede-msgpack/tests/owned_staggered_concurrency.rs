...
fn multi_field_struct_byte_at_a_time() {
    ...
    let delta_key = fixstr("delta");
    let gamma_key = fixstr("gamma");
    let beta_key = fixstr("beta");
    let alpha_key = fixstr("alpha");
    let beta_val = fixstr("hi");
    let msg = build_map(&[
        (delta_key.as_slice(), &[4u8]),
        (gamma_key.as_slice(), &[0xc3]), // true
        (beta_key.as_slice(), beta_val.as_slice()),
        (alpha_key.as_slice(), &[1u8]),
    ]);
