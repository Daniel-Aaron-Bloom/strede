...
pub trait Entry<'de>: Sized {
    ...
    /// Attempt a **zero-copy** number borrow.
    ///
    /// Hits only when the format can return the entire number as a single
    /// contiguous `&'de str` slice. Returns `Ok(Probe::Miss)` when the
    /// token is a number but cannot be represented as a single slice (e.g.
    /// a number split across streaming chunks), so that a concurrent
    /// [`Entry::deserialize_number_chunks`] arm can take over via
    /// `select_probe!`.
    ///
    /// `Ok(Probe::Miss)` on type mismatch *or* when zero-copy is unavailable;
    /// `Err` on fatal format error only.
    async fn deserialize_number(self) -> Result<Probe<(Self::Claim, &'de str)>, Self::Error>;
    /// Begin streaming a number chunk-by-chunk. The [`Entry::Claim`] is
    /// returned by [`NumberAccess::next_number_chunk`] when the number is
    /// exhausted. Handles all numbers including those split across buffer
    /// boundaries.
    /// `Ok(Probe::Miss)` on type mismatch; `Err` on fatal format error only.
...
/// Streams a number's textual representation in zero-copy chunks.
/// Obtained from [`Entry::deserialize_number_chunks`].
///
/// Numbers are primitives — the type is already known, so no probing or
/// racing is needed. This accessor yields the number's digits (including
/// sign, decimal point, and exponent) as `&str` chunks. Useful for
/// arbitrary-precision number types that cannot be represented by fixed
/// primitives like `i64` or `f64`.
///
/// ```rust,ignore
...
///
/// Terminal case: [`crate::FlattenTerminal`] calls `map.iterate(SkipUnknown!(arms))`.
...
/// continuation. For the terminal case ([`crate::FlattenTerminal`]) this drives the
