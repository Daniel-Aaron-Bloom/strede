use strede::owned::NumberAccessOwned;
use strede::{Chunk, Probe};

/// A decimal floating-point number, represented as a sequence of decimal digits.
#[derive(Clone, Debug, PartialEq)]
pub(super) struct DecimalSeq {
    /// The number of significant digits in the decimal.
    pub num_digits: usize,
    /// The offset of the decimal point in the significant digits.
    pub decimal_point: i32,
    /// If the number of significant digits stored in the decimal is truncated.
    pub truncated: bool,
    /// Buffer of the raw digits, in the range [0, 9].
    pub digits: [u8; Self::MAX_DIGITS],
}

impl Default for DecimalSeq {
    fn default() -> Self {
        Self {
            num_digits: 0,
            decimal_point: 0,
            truncated: false,
            digits: [0; Self::MAX_DIGITS],
        }
    }
}

impl DecimalSeq {
    /// The maximum number of digits required to unambiguously round up to a 64-bit float.
    ///
    /// For an IEEE 754 binary64 float, this required 767 digits. So we store the max digits + 1.
    ///
    /// We can exactly represent a float in radix `b` from radix 2 if
    /// `b` is divisible by 2. This function calculates the exact number of
    /// digits required to exactly represent that float.
    ///
    /// According to the "Handbook of Floating Point Arithmetic",
    /// for IEEE754, with `emin` being the min exponent, `p2` being the
    /// precision, and `b` being the radix, the number of digits follows as:
    ///
    /// `−emin + p2 + ⌊(emin + 1) log(2, b) − log(1 − 2^(−p2), b)⌋`
    ///
    /// For f32, this follows as:
    ///     emin = -126
    ///     p2 = 24
    ///
    /// For f64, this follows as:
    ///     emin = -1022
    ///     p2 = 53
    ///
    /// In Python:
    ///     `-emin + p2 + math.floor((emin+ 1)*math.log(2, b)-math.log(1-2**(-p2), b))`
    pub const MAX_DIGITS: usize = 768;

    /// The max decimal digits that can be exactly represented in a 64-bit integer.
    pub(super) const MAX_DIGITS_WITHOUT_OVERFLOW: usize = 19;
    pub(super) const DECIMAL_POINT_RANGE: i32 = 2047;

    /// Append a digit to the buffer if it fits.
    pub(super) fn try_add_digit(&mut self, digit: u8) {
        if self.num_digits < Self::MAX_DIGITS {
            self.digits[self.num_digits] = digit;
        }
        self.num_digits += 1;
    }

    /// Trim trailing zeros from the buffer.
    pub fn trim(&mut self) {
        // All of the following calls to `DecimalSeq::trim` can't panic because:
        //
        //  1. `parse_decimal` sets `num_digits` to a max of `DecimalSeq::MAX_DIGITS`.
        //  2. `right_shift` sets `num_digits` to `write_index`, which is bounded by `num_digits`.
        //  3. `left_shift` `num_digits` to a max of `DecimalSeq::MAX_DIGITS`.
        //
        // Trim is only called in `right_shift` and `left_shift`.
        debug_assert!(self.num_digits <= Self::MAX_DIGITS);
        while self.num_digits != 0 && self.digits[self.num_digits - 1] == 0 {
            self.num_digits -= 1;
        }
    }

    pub(super) fn round(&self) -> u64 {
        if self.num_digits == 0 || self.decimal_point < 0 {
            return 0;
        } else if self.decimal_point >= Self::MAX_DIGITS_WITHOUT_OVERFLOW as i32 {
            return 0xFFFF_FFFF_FFFF_FFFF_u64;
        }

        let dp = self.decimal_point as usize;
        let mut n = 0_u64;

        for i in 0..dp {
            n *= 10;
            if i < self.num_digits {
                n += self.digits[i] as u64;
            }
        }

        let mut round_up = false;

        if dp < self.num_digits {
            round_up = self.digits[dp] >= 5;
            if self.digits[dp] == 5 && dp + 1 == self.num_digits {
                round_up = self.truncated || ((dp != 0) && (1 & self.digits[dp - 1] != 0))
            }
        }

        if round_up {
            n += 1;
        }
        n
    }

    /// Computes decimal * 2^shift.
    pub(super) fn left_shift(&mut self, shift: usize) {
        if self.num_digits == 0 {
            return;
        }
        let num_new_digits = number_of_digits_decimal_left_shift(self, shift);
        let mut read_index = self.num_digits;
        let mut write_index = self.num_digits + num_new_digits;
        let mut n = 0_u64;

        while read_index != 0 {
            read_index -= 1;
            write_index -= 1;
            n += (self.digits[read_index] as u64) << shift;
            let quotient = n / 10;
            let remainder = n - (10 * quotient);
            if write_index < Self::MAX_DIGITS {
                self.digits[write_index] = remainder as u8;
            } else if remainder > 0 {
                self.truncated = true;
            }
            n = quotient;
        }

        while n > 0 {
            write_index -= 1;
            let quotient = n / 10;
            let remainder = n - (10 * quotient);
            if write_index < Self::MAX_DIGITS {
                self.digits[write_index] = remainder as u8;
            } else if remainder > 0 {
                self.truncated = true;
            }
            n = quotient;
        }

        self.num_digits += num_new_digits;

        if self.num_digits > Self::MAX_DIGITS {
            self.num_digits = Self::MAX_DIGITS;
        }

        self.decimal_point += num_new_digits as i32;
        self.trim();
    }

    /// Computes decimal * 2^-shift.
    pub(super) fn right_shift(&mut self, shift: usize) {
        let mut read_index = 0;
        let mut write_index = 0;
        let mut n = 0_u64;
        while (n >> shift) == 0 {
            if read_index < self.num_digits {
                n = (10 * n) + self.digits[read_index] as u64;
                read_index += 1;
            } else if n == 0 {
                return;
            } else {
                while (n >> shift) == 0 {
                    n *= 10;
                    read_index += 1;
                }
                break;
            }
        }
        self.decimal_point -= read_index as i32 - 1;
        if self.decimal_point < -Self::DECIMAL_POINT_RANGE {
            // `self = Self::Default()`, but without the overhead of clearing `digits`.
            self.num_digits = 0;
            self.decimal_point = 0;
            self.truncated = false;
            return;
        }
        let mask = (1_u64 << shift) - 1;
        while read_index < self.num_digits {
            let new_digit = (n >> shift) as u8;
            n = (10 * (n & mask)) + self.digits[read_index] as u64;
            read_index += 1;
            self.digits[write_index] = new_digit;
            write_index += 1;
        }
        while n > 0 {
            let new_digit = (n >> shift) as u8;
            n = 10 * (n & mask);
            if write_index < Self::MAX_DIGITS {
                self.digits[write_index] = new_digit;
                write_index += 1;
            } else if new_digit > 0 {
                self.truncated = true;
            }
        }
        self.num_digits = write_index;
        self.trim();
    }
}

trait ByteSlice {
    /// Reads 8 bytes as a 64-bit integer in little-endian order.
    fn read_u64(&self) -> u64;

    /// Writes a 64-bit integer as 8 bytes in little-endian order.
    fn write_u64(&mut self, value: u64);
}

impl ByteSlice for [u8] {
    #[inline(always)] // inlining this is crucial to remove bound checks
    fn read_u64(&self) -> u64 {
        let mut tmp = [0; 8];
        tmp.copy_from_slice(&self[..8]);
        u64::from_le_bytes(tmp)
    }

    #[inline(always)] // inlining this is crucial to remove bound checks
    fn write_u64(&mut self, value: u64) {
        self[..8].copy_from_slice(&value.to_le_bytes())
    }
}

/// Parse 8 digits from a little-endian `u64` in 3 multiplications.
///
/// See: <https://johnnylee-sde.github.io/Fast-numeric-string-to-int/>
fn parse_8digits(mut v: u64) -> u64 {
    const MASK: u64 = 0x0000_00FF_0000_00FF;
    const MUL1: u64 = 0x000F_4240_0000_0064;
    const MUL2: u64 = 0x0000_2710_0000_0001;
    v -= 0x3030_3030_3030_3030;
    v = (v * 10) + (v >> 8); // will not overflow, fits in 63 bits
    let v1 = (v & MASK).wrapping_mul(MUL1);
    let v2 = ((v >> 16) & MASK).wrapping_mul(MUL2);
    ((v1.wrapping_add(v2) >> 32) as u32) as u64
}

/// Determine if all characters in an 8-byte byte string (represented as a `u64`) are all decimal
/// digits.
///
/// This does not care about the order in which the bytes were loaded.
pub(crate) fn is_8digits(v: u64) -> bool {
    let a = v.wrapping_add(0x4646_4646_4646_4646);
    let b = v.wrapping_sub(0x3030_3030_3030_3030);
    (a | b) & 0x8080_8080_8080_8080 == 0
}

#[derive(Debug, PartialEq, Eq, Clone, Copy)]
enum SeqParseState {
    LeadingSign,
    LeadingZeroes,
    PreDecimalDigits,
    Decimal,
    PostDecimalLeadingZeroes,
    PostDecimalDigits { trailing_zeros: usize },
    ExponentStart,
    ExponentSign,
    ExponentDigits { neg_exp: bool, exp_num: Option<i32> },
}

/// Parsed representation produced by [`parse_decimal_fast`].
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(super) struct DecimalFast {
    /// Decimal exponent: `mantissa * 10^exponent`.
    pub exponent: i64,
    /// Up to 19 significant digits, packed into a u64.
    pub mantissa: u64,
    /// True when more than 19 significant digits were present (fast path cannot
    /// be trusted).
    pub many_digits: bool,
}

#[derive(Debug, PartialEq, Eq, Clone, Copy)]
enum FastParseState {
    LeadingSign,
    PreDecimalDigits,
    Decimal,
    DecimalDigits,
    ExponentSign,
    ExponentDigits,
}

/// Streaming fast-path parser: accumulates up to 19 significant digits and the
/// decimal exponent.
///
/// Returns `Probe::Miss` when the input is an integer (no `.`, `e`, or `E`),
/// or when the exponent is malformed.
pub(super) async fn parse_decimal_fast<A: NumberAccessOwned>(
    mut chunks: A,
) -> Result<Probe<(A::Claim, DecimalFast, bool)>, A::Error> {
    let mut state = FastParseState::LeadingSign;
    let mut neg = false;
    let mut mantissa: u64 = 0;
    let mut n_digits: i64 = 0;
    let mut n_after_dot: i64 = 0;
    // Digits after the decimal that were actually folded into mantissa.
    // Used for the exponent; n_after_dot continues counting all digits for many_digits.
    let mut n_after_dot_in_mantissa: i64 = 0;
    let mut exp_number: i64 = 0;
    let mut exp_neg = false;
    let mut exp_has_digit = false;

    loop {
        match chunks
            .next_number_chunk(|s| {
                let mut s = s.as_bytes();
                if state == FastParseState::LeadingSign {
                    if let Some((&b'-', rest)) = s.split_first() {
                        neg = true;
                        s = rest;
                    }
                    if s.is_empty() {
                        return true;
                    }
                    state = FastParseState::PreDecimalDigits;
                }
                if state == FastParseState::PreDecimalDigits {
                    while s.len() >= 8 && n_digits + 8 <= 19 {
                        let v = s.read_u64();
                        if !is_8digits(v) {
                            break;
                        }
                        mantissa = mantissa
                            .wrapping_mul(1_0000_0000)
                            .wrapping_add(parse_8digits(v));
                        n_digits += 8;
                        s = &s[8..];
                    }
                    while let Some((&b, rest)) = s.split_first() {
                        let c = b.wrapping_sub(b'0');
                        if c >= 10 {
                            break;
                        }
                        if n_digits < 19 {
                            mantissa = mantissa.wrapping_mul(10).wrapping_add(c as u64);
                        }
                        n_digits += 1;
                        s = rest;
                    }
                    if s.is_empty() {
                        return true;
                    }
                    state = FastParseState::Decimal;
                }
                if state == FastParseState::Decimal {
                    if s.is_empty() {
                        return true;
                    }
                    match s.split_first() {
                        Some((&b'.', rest)) => {
                            s = rest;
                            state = FastParseState::DecimalDigits;
                        }

                        Some((&(b'e' | b'E'), rest)) => {
                            s = rest;
                            state = FastParseState::ExponentSign;
                        }
                        _ => return false,
                    }
                }
                if state == FastParseState::DecimalDigits {
                    while s.len() >= 8 && n_digits + n_after_dot + 8 <= 19 {
                        let v = s.read_u64();
                        if !is_8digits(v) {
                            break;
                        }
                        mantissa = mantissa
                            .wrapping_mul(1_0000_0000)
                            .wrapping_add(parse_8digits(v));
                        n_after_dot += 8;
                        n_after_dot_in_mantissa += 8;
                        s = &s[8..];
                    }
                    while let Some((&b, rest)) = s.split_first() {
                        let c = b.wrapping_sub(b'0');
                        if c >= 10 {
                            break;
                        }
                        if n_digits + n_after_dot < 19 {
                            mantissa = mantissa.wrapping_mul(10).wrapping_add(c as u64);
                            n_after_dot_in_mantissa += 1;
                        }
                        n_after_dot += 1;
                        s = rest;
                    }
                    if s.is_empty() {
                        return true;
                    }
                    if let Some((&b'e' | b'E', rest)) = s.split_first() {
                        s = rest;
                        state = FastParseState::ExponentSign;
                    } else {
                        return false;
                    }
                }
                if state == FastParseState::ExponentSign {
                    if s.is_empty() {
                        return true;
                    }
                    if let Some((&ch @ (b'-' | b'+'), rest)) = s.split_first() {
                        exp_neg = ch == b'-';
                        s = rest;
                    }
                    state = FastParseState::ExponentDigits;
                }
                if state == FastParseState::ExponentDigits {
                    while let Some((&b, rest)) = s.split_first() {
                        let digit = b.wrapping_sub(b'0');
                        if digit >= 10 {
                            return false;
                        }
                        if exp_number < 0x10000 {
                            exp_number = 10 * exp_number + digit as i64;
                        }
                        exp_has_digit = true;
                        s = rest;
                    }
                    return s.is_empty();
                }
                true
            })
            .await?
        {
            Chunk::Data((next, true)) => chunks = next,
            Chunk::Data((_, false)) => return Ok(Probe::Miss),
            Chunk::Done(claim) => {
                match state {
                    FastParseState::LeadingSign
                    | FastParseState::PreDecimalDigits
                    | FastParseState::Decimal => return Ok(Probe::Miss),
                    FastParseState::ExponentSign => return Ok(Probe::Miss),
                    FastParseState::ExponentDigits if !exp_has_digit => return Ok(Probe::Miss),
                    _ => {}
                }
                let pre_decimal_truncated = (n_digits - 19).max(0);
                let exponent = (if exp_neg { -exp_number } else { exp_number })
                    + pre_decimal_truncated
                    - n_after_dot_in_mantissa;
                let n_digits = n_digits + n_after_dot;
                let many_digits = n_digits > 19;
                return Ok(Probe::Hit((
                    claim,
                    DecimalFast {
                        exponent,
                        mantissa,
                        many_digits,
                    },
                    neg,
                )));
            }
        }
    }
}

/// Streaming `parse_decimal_seq`: builds a [`DecimalSeq`] from a chunked
/// number stream. Returns `Probe::Miss` if the input has no `.`, `e`, or `E`
/// (handing the win to one of the int contestants).
pub(super) async fn parse_decimal_seq<A: NumberAccessOwned>(
    mut chunks: A,
) -> Result<Probe<(A::Claim, DecimalSeq, bool)>, A::Error> {
    let mut d = DecimalSeq::default();
    let mut state = SeqParseState::LeadingSign;
    let mut neg = false;

    loop {
        match chunks
            .next_number_chunk(|s| {
                let mut s = s.as_bytes();
                if state == SeqParseState::LeadingSign {
                    while let Some((&b'-', rest)) = s.split_first() {
                        neg = true;
                        s = rest;
                    }
                    if s.is_empty() {
                        return true;
                    }
                    state = SeqParseState::LeadingZeroes;
                }
                if state == SeqParseState::LeadingZeroes {
                    while let Some((&b'0', rest)) = s.split_first() {
                        s = rest;
                    }
                    if s.is_empty() {
                        return true;
                    }
                    state = SeqParseState::PreDecimalDigits;
                }
                if state == SeqParseState::PreDecimalDigits {
                    while let Some((&b, rest)) = s.split_first() {
                        let c = b.wrapping_sub(b'0');
                        if c >= 10 {
                            break;
                        }
                        d.try_add_digit(c);
                        s = rest;
                    }
                    if s.is_empty() {
                        return true;
                    }
                    state = SeqParseState::Decimal;
                }
                if state == SeqParseState::Decimal {
                    if s.is_empty() {
                        return true;
                    }
                    d.decimal_point = d.num_digits as i32;
                    if let Some((b'.', s_next)) = s.split_first() {
                        s = s_next;

                        state = if d.num_digits == 0 {
                            SeqParseState::PostDecimalLeadingZeroes
                        } else {
                            SeqParseState::PostDecimalDigits { trailing_zeros: 0 }
                        }
                    } else {
                        state = SeqParseState::ExponentStart;
                    }
                }
                if state == SeqParseState::PostDecimalLeadingZeroes {
                    // Skip leading zeros.
                    while let Some((&b'0', s_next)) = s.split_first() {
                        s = s_next;
                        d.decimal_point -= 1; // count skipped leading zeros
                    }
                    state = SeqParseState::PostDecimalDigits { trailing_zeros: 0 };
                }
                if let SeqParseState::PostDecimalDigits { mut trailing_zeros } = state {
                    while s.len() >= 8 && d.num_digits + 8 < DecimalSeq::MAX_DIGITS {
                        let v = s.read_u64();
                        if !is_8digits(v) {
                            break;
                        }
                        let v = v - 0x3030_3030_3030_3030;
                        trailing_zeros = if v == 0 {
                            trailing_zeros + 8
                        } else {
                            // v is little-endian: the last digit read sits in the
                            // high byte, so trailing zero digits == leading zero
                            // bytes of v.
                            v.leading_zeros() as usize / 8
                        };
                        d.digits[d.num_digits..].write_u64(v);
                        d.num_digits += 8;
                        s = &s[8..];
                    }
                    while let Some((&b, rest)) = s.split_first() {
                        let c = b.wrapping_sub(b'0');
                        if c >= 10 {
                            break;
                        }

                        trailing_zeros = if c == 0 { trailing_zeros + 1 } else { 0 };
                        d.try_add_digit(c);
                        s = rest;
                    }
                    if s.is_empty() {
                        state = SeqParseState::PostDecimalDigits { trailing_zeros };
                        return true;
                    }

                    if d.num_digits != 0 {
                        d.num_digits -= trailing_zeros;
                        if d.num_digits > DecimalSeq::MAX_DIGITS {
                            d.truncated = true;
                            d.num_digits = DecimalSeq::MAX_DIGITS;
                        }
                    }
                    state = SeqParseState::ExponentStart;
                }
                if state == SeqParseState::ExponentStart {
                    if s.is_empty() {
                        return true;
                    }
                    if let Some((&b'e' | b'E', s_next)) = s.split_first() {
                        s = s_next;
                        state = SeqParseState::ExponentSign;
                    } else {
                        // Validation failed
                        return false;
                    }
                }
                if state == SeqParseState::ExponentSign {
                    if s.is_empty() {
                        return true;
                    }
                    let mut neg_exp = false;
                    if let Some((&ch @ (b'-' | b'+'), s_next)) = s.split_first() {
                        neg_exp = ch == b'-';
                        s = s_next;
                    }
                    state = SeqParseState::ExponentDigits {
                        neg_exp,
                        exp_num: None,
                    };
                }
                debug_assert!(matches!(state, SeqParseState::ExponentDigits { .. }));

                let SeqParseState::ExponentDigits {
                    neg_exp,
                    mut exp_num,
                } = state
                else {
                    return false;
                };

                while let Some((&b, rest)) = s.split_first() {
                    let digit = b.wrapping_sub(b'0');
                    let cur = exp_num.unwrap_or(0);
                    if digit >= 10 || cur >= 0x10000 {
                        return false;
                    }
                    exp_num = Some(10 * cur + digit as i32);
                    s = rest;
                }
                state = SeqParseState::ExponentDigits { neg_exp, exp_num };
                s.is_empty()
            })
            .await?
        {
            Chunk::Data((next, true)) => chunks = next,
            Chunk::Data((_, false)) => return Ok(Probe::Miss),
            Chunk::Done(claim) => {
                match state {
                    // No '.' or 'e'/'E' seen — hand off to integer path.
                    SeqParseState::LeadingSign
                    | SeqParseState::LeadingZeroes
                    | SeqParseState::PreDecimalDigits
                    | SeqParseState::Decimal => return Ok(Probe::Miss),
                    // 'e'/'E' seen but no exponent digits — malformed.
                    SeqParseState::ExponentSign => return Ok(Probe::Miss),
                    // After '.': all digits were leading zeros (e.g. "0.000").
                    // decimal_point is already adjusted; nothing to trim.
                    SeqParseState::PostDecimalLeadingZeroes => {}
                    // After post-decimal digits: strip accumulated trailing zeros.
                    SeqParseState::PostDecimalDigits { trailing_zeros } => {
                        if d.num_digits != 0 {
                            d.num_digits -= trailing_zeros;
                            if d.num_digits > DecimalSeq::MAX_DIGITS {
                                d.truncated = true;
                                d.num_digits = DecimalSeq::MAX_DIGITS;
                            }
                        }
                    }
                    // No exponent (terminated at 'e'/'E' decision point).
                    SeqParseState::ExponentStart => {}
                    // Exponent present: apply it. None means sign seen but no
                    // digits (e.g. "1.5e+"), which is malformed.
                    SeqParseState::ExponentDigits { exp_num: None, .. } => return Ok(Probe::Miss),
                    SeqParseState::ExponentDigits {
                        neg_exp,
                        exp_num: Some(n),
                    } => {
                        d.decimal_point += if neg_exp { -n } else { n };
                    }
                }
                return Ok(Probe::Hit((claim, d, neg)));
            }
        }
    }
}

fn number_of_digits_decimal_left_shift(d: &DecimalSeq, mut shift: usize) -> usize {
    #[rustfmt::skip]
    const TABLE: [u16; 65] = [
        0x0000, 0x0800, 0x0801, 0x0803, 0x1006, 0x1009, 0x100D, 0x1812, 0x1817, 0x181D, 0x2024,
        0x202B, 0x2033, 0x203C, 0x2846, 0x2850, 0x285B, 0x3067, 0x3073, 0x3080, 0x388E, 0x389C,
        0x38AB, 0x38BB, 0x40CC, 0x40DD, 0x40EF, 0x4902, 0x4915, 0x4929, 0x513E, 0x5153, 0x5169,
        0x5180, 0x5998, 0x59B0, 0x59C9, 0x61E3, 0x61FD, 0x6218, 0x6A34, 0x6A50, 0x6A6D, 0x6A8B,
        0x72AA, 0x72C9, 0x72E9, 0x7B0A, 0x7B2B, 0x7B4D, 0x8370, 0x8393, 0x83B7, 0x83DC, 0x8C02,
        0x8C28, 0x8C4F, 0x9477, 0x949F, 0x94C8, 0x9CF2, 0x051C, 0x051C, 0x051C, 0x051C,
    ];
    #[rustfmt::skip]
    const TABLE_POW5: [u8; 0x051C] = [
        5, 2, 5, 1, 2, 5, 6, 2, 5, 3, 1, 2, 5, 1, 5, 6, 2, 5, 7, 8, 1, 2, 5, 3, 9, 0, 6, 2, 5, 1,
        9, 5, 3, 1, 2, 5, 9, 7, 6, 5, 6, 2, 5, 4, 8, 8, 2, 8, 1, 2, 5, 2, 4, 4, 1, 4, 0, 6, 2, 5,
        1, 2, 2, 0, 7, 0, 3, 1, 2, 5, 6, 1, 0, 3, 5, 1, 5, 6, 2, 5, 3, 0, 5, 1, 7, 5, 7, 8, 1, 2,
        5, 1, 5, 2, 5, 8, 7, 8, 9, 0, 6, 2, 5, 7, 6, 2, 9, 3, 9, 4, 5, 3, 1, 2, 5, 3, 8, 1, 4, 6,
        9, 7, 2, 6, 5, 6, 2, 5, 1, 9, 0, 7, 3, 4, 8, 6, 3, 2, 8, 1, 2, 5, 9, 5, 3, 6, 7, 4, 3, 1,
        6, 4, 0, 6, 2, 5, 4, 7, 6, 8, 3, 7, 1, 5, 8, 2, 0, 3, 1, 2, 5, 2, 3, 8, 4, 1, 8, 5, 7, 9,
        1, 0, 1, 5, 6, 2, 5, 1, 1, 9, 2, 0, 9, 2, 8, 9, 5, 5, 0, 7, 8, 1, 2, 5, 5, 9, 6, 0, 4, 6,
        4, 4, 7, 7, 5, 3, 9, 0, 6, 2, 5, 2, 9, 8, 0, 2, 3, 2, 2, 3, 8, 7, 6, 9, 5, 3, 1, 2, 5, 1,
        4, 9, 0, 1, 1, 6, 1, 1, 9, 3, 8, 4, 7, 6, 5, 6, 2, 5, 7, 4, 5, 0, 5, 8, 0, 5, 9, 6, 9, 2,
        3, 8, 2, 8, 1, 2, 5, 3, 7, 2, 5, 2, 9, 0, 2, 9, 8, 4, 6, 1, 9, 1, 4, 0, 6, 2, 5, 1, 8, 6,
        2, 6, 4, 5, 1, 4, 9, 2, 3, 0, 9, 5, 7, 0, 3, 1, 2, 5, 9, 3, 1, 3, 2, 2, 5, 7, 4, 6, 1, 5,
        4, 7, 8, 5, 1, 5, 6, 2, 5, 4, 6, 5, 6, 6, 1, 2, 8, 7, 3, 0, 7, 7, 3, 9, 2, 5, 7, 8, 1, 2,
        5, 2, 3, 2, 8, 3, 0, 6, 4, 3, 6, 5, 3, 8, 6, 9, 6, 2, 8, 9, 0, 6, 2, 5, 1, 1, 6, 4, 1, 5,
        3, 2, 1, 8, 2, 6, 9, 3, 4, 8, 1, 4, 4, 5, 3, 1, 2, 5, 5, 8, 2, 0, 7, 6, 6, 0, 9, 1, 3, 4,
        6, 7, 4, 0, 7, 2, 2, 6, 5, 6, 2, 5, 2, 9, 1, 0, 3, 8, 3, 0, 4, 5, 6, 7, 3, 3, 7, 0, 3, 6,
        1, 3, 2, 8, 1, 2, 5, 1, 4, 5, 5, 1, 9, 1, 5, 2, 2, 8, 3, 6, 6, 8, 5, 1, 8, 0, 6, 6, 4, 0,
        6, 2, 5, 7, 2, 7, 5, 9, 5, 7, 6, 1, 4, 1, 8, 3, 4, 2, 5, 9, 0, 3, 3, 2, 0, 3, 1, 2, 5, 3,
        6, 3, 7, 9, 7, 8, 8, 0, 7, 0, 9, 1, 7, 1, 2, 9, 5, 1, 6, 6, 0, 1, 5, 6, 2, 5, 1, 8, 1, 8,
        9, 8, 9, 4, 0, 3, 5, 4, 5, 8, 5, 6, 4, 7, 5, 8, 3, 0, 0, 7, 8, 1, 2, 5, 9, 0, 9, 4, 9, 4,
        7, 0, 1, 7, 7, 2, 9, 2, 8, 2, 3, 7, 9, 1, 5, 0, 3, 9, 0, 6, 2, 5, 4, 5, 4, 7, 4, 7, 3, 5,
        0, 8, 8, 6, 4, 6, 4, 1, 1, 8, 9, 5, 7, 5, 1, 9, 5, 3, 1, 2, 5, 2, 2, 7, 3, 7, 3, 6, 7, 5,
        4, 4, 3, 2, 3, 2, 0, 5, 9, 4, 7, 8, 7, 5, 9, 7, 6, 5, 6, 2, 5, 1, 1, 3, 6, 8, 6, 8, 3, 7,
        7, 2, 1, 6, 1, 6, 0, 2, 9, 7, 3, 9, 3, 7, 9, 8, 8, 2, 8, 1, 2, 5, 5, 6, 8, 4, 3, 4, 1, 8,
        8, 6, 0, 8, 0, 8, 0, 1, 4, 8, 6, 9, 6, 8, 9, 9, 4, 1, 4, 0, 6, 2, 5, 2, 8, 4, 2, 1, 7, 0,
        9, 4, 3, 0, 4, 0, 4, 0, 0, 7, 4, 3, 4, 8, 4, 4, 9, 7, 0, 7, 0, 3, 1, 2, 5, 1, 4, 2, 1, 0,
        8, 5, 4, 7, 1, 5, 2, 0, 2, 0, 0, 3, 7, 1, 7, 4, 2, 2, 4, 8, 5, 3, 5, 1, 5, 6, 2, 5, 7, 1,
        0, 5, 4, 2, 7, 3, 5, 7, 6, 0, 1, 0, 0, 1, 8, 5, 8, 7, 1, 1, 2, 4, 2, 6, 7, 5, 7, 8, 1, 2,
        5, 3, 5, 5, 2, 7, 1, 3, 6, 7, 8, 8, 0, 0, 5, 0, 0, 9, 2, 9, 3, 5, 5, 6, 2, 1, 3, 3, 7, 8,
        9, 0, 6, 2, 5, 1, 7, 7, 6, 3, 5, 6, 8, 3, 9, 4, 0, 0, 2, 5, 0, 4, 6, 4, 6, 7, 7, 8, 1, 0,
        6, 6, 8, 9, 4, 5, 3, 1, 2, 5, 8, 8, 8, 1, 7, 8, 4, 1, 9, 7, 0, 0, 1, 2, 5, 2, 3, 2, 3, 3,
        8, 9, 0, 5, 3, 3, 4, 4, 7, 2, 6, 5, 6, 2, 5, 4, 4, 4, 0, 8, 9, 2, 0, 9, 8, 5, 0, 0, 6, 2,
        6, 1, 6, 1, 6, 9, 4, 5, 2, 6, 6, 7, 2, 3, 6, 3, 2, 8, 1, 2, 5, 2, 2, 2, 0, 4, 4, 6, 0, 4,
        9, 2, 5, 0, 3, 1, 3, 0, 8, 0, 8, 4, 7, 2, 6, 3, 3, 3, 6, 1, 8, 1, 6, 4, 0, 6, 2, 5, 1, 1,
        1, 0, 2, 2, 3, 0, 2, 4, 6, 2, 5, 1, 5, 6, 5, 4, 0, 4, 2, 3, 6, 3, 1, 6, 6, 8, 0, 9, 0, 8,
        2, 0, 3, 1, 2, 5, 5, 5, 5, 1, 1, 1, 5, 1, 2, 3, 1, 2, 5, 7, 8, 2, 7, 0, 2, 1, 1, 8, 1, 5,
        8, 3, 4, 0, 4, 5, 4, 1, 0, 1, 5, 6, 2, 5, 2, 7, 7, 5, 5, 5, 7, 5, 6, 1, 5, 6, 2, 8, 9, 1,
        3, 5, 1, 0, 5, 9, 0, 7, 9, 1, 7, 0, 2, 2, 7, 0, 5, 0, 7, 8, 1, 2, 5, 1, 3, 8, 7, 7, 7, 8,
        7, 8, 0, 7, 8, 1, 4, 4, 5, 6, 7, 5, 5, 2, 9, 5, 3, 9, 5, 8, 5, 1, 1, 3, 5, 2, 5, 3, 9, 0,
        6, 2, 5, 6, 9, 3, 8, 8, 9, 3, 9, 0, 3, 9, 0, 7, 2, 2, 8, 3, 7, 7, 6, 4, 7, 6, 9, 7, 9, 2,
        5, 5, 6, 7, 6, 2, 6, 9, 5, 3, 1, 2, 5, 3, 4, 6, 9, 4, 4, 6, 9, 5, 1, 9, 5, 3, 6, 1, 4, 1,
        8, 8, 8, 2, 3, 8, 4, 8, 9, 6, 2, 7, 8, 3, 8, 1, 3, 4, 7, 6, 5, 6, 2, 5, 1, 7, 3, 4, 7, 2,
        3, 4, 7, 5, 9, 7, 6, 8, 0, 7, 0, 9, 4, 4, 1, 1, 9, 2, 4, 4, 8, 1, 3, 9, 1, 9, 0, 6, 7, 3,
        8, 2, 8, 1, 2, 5, 8, 6, 7, 3, 6, 1, 7, 3, 7, 9, 8, 8, 4, 0, 3, 5, 4, 7, 2, 0, 5, 9, 6, 2,
        2, 4, 0, 6, 9, 5, 9, 5, 3, 3, 6, 9, 1, 4, 0, 6, 2, 5,
    ];

    shift &= 63;
    let x_a = TABLE[shift];
    let x_b = TABLE[shift + 1];
    let num_new_digits = (x_a >> 11) as _;
    let pow5_a = (0x7FF & x_a) as usize;
    let pow5_b = (0x7FF & x_b) as usize;
    let pow5 = &TABLE_POW5[pow5_a..];

    for (i, &p5) in pow5.iter().enumerate().take(pow5_b - pow5_a) {
        if i >= d.num_digits {
            return num_new_digits - 1;
        } else if d.digits[i] == p5 {
            continue;
        } else if d.digits[i] < p5 {
            return num_new_digits - 1;
        } else {
            return num_new_digits;
        }
    }

    num_new_digits
}

// ===========================================================================
// Tests
// ===========================================================================

#[cfg(test)]
mod tests {
    use super::*;
    extern crate std;
    use strede_test_util::block_on;

    // -----------------------------------------------------------------------
    // Minimal fake NumberAccessOwned
    //
    // Feeds the number string as a sequence of chunks split at arbitrary
    // byte positions (driven by `chunk_size`). Each call to
    // `next_number_chunk` returns one chunk of `chunk_size` bytes until the
    // string is exhausted, at which point it returns `Chunk::Done(())`.
    // -----------------------------------------------------------------------

    #[derive(Clone)]
    struct FakeAccess<'a> {
        remaining: &'a str,
        chunk_size: usize,
    }

    impl<'a> FakeAccess<'a> {
        fn new(s: &'a str) -> Self {
            Self {
                remaining: s,
                chunk_size: s.len().max(1),
            }
        }

        fn with_chunk_size(s: &'a str, chunk_size: usize) -> Self {
            Self {
                remaining: s,
                chunk_size: chunk_size.max(1),
            }
        }
    }

    #[derive(Debug)]
    struct Never;
    impl strede::DeserializeError for Never {
        fn duplicate_field(_: &'static str) -> Self {
            Never
        }
    }

    impl<'a> strede::owned::NumberAccessOwned for FakeAccess<'a> {
        type Claim = ();
        type Error = Never;

        fn fork(&mut self) -> Self {
            self.clone()
        }

        async fn next_number_chunk<R>(
            mut self,
            f: impl FnOnce(&str) -> R,
        ) -> Result<Chunk<(Self, R), ()>, Never> {
            if self.remaining.is_empty() {
                // Already exhausted — return Done with a trivial claim.
                // We need to call f with an empty slice so we don't skip it,
                // but Done signals end-of-stream.
                return Ok(Chunk::Done(()));
            }
            let split = self.chunk_size.min(self.remaining.len());
            let (chunk, rest) = self.remaining.split_at(split);
            let r = f(chunk);
            self.remaining = rest;
            Ok(Chunk::Data((self, r)))
        }
    }

    // -----------------------------------------------------------------------
    // Helper: run parse_decimal_seq and return the DecimalSeq or None on Miss
    // -----------------------------------------------------------------------

    fn parse(s: &str) -> Option<(DecimalSeq, bool)> {
        block_on(parse_decimal_seq(FakeAccess::new(s)))
            .ok()
            .and_then(|p| match p {
                Probe::Hit((_, d, neg)) => Some((d, neg)),
                Probe::Miss => None,
            })
    }

    fn parse_chunked(s: &str, chunk_size: usize) -> Option<(DecimalSeq, bool)> {
        block_on(parse_decimal_seq(FakeAccess::with_chunk_size(
            s, chunk_size,
        )))
        .ok()
        .and_then(|p| match p {
            Probe::Hit((_, d, neg)) => Some((d, neg)),
            Probe::Miss => None,
        })
    }

    // -----------------------------------------------------------------------
    // Miss cases — integers have no '.' or 'e'
    // -----------------------------------------------------------------------

    #[test]
    fn miss_on_integer() {
        assert!(parse("42").is_none());
        assert!(parse("-7").is_none());
        assert!(parse("0").is_none());
    }

    // -----------------------------------------------------------------------
    // Basic decimal: "1.5"
    // -----------------------------------------------------------------------

    #[test]
    fn basic_decimal() {
        let (d, neg) = parse("1.5").unwrap();
        assert!(!neg);
        // digits: [1, 5], decimal_point = 1
        assert_eq!(d.decimal_point, 1);
        assert_eq!(d.num_digits, 2);
        assert_eq!(&d.digits[..2], &[1, 5]);
    }

    // -----------------------------------------------------------------------
    // Negative sign
    // -----------------------------------------------------------------------

    #[test]
    fn negative_decimal() {
        let (d, neg) = parse("-3.14").unwrap();
        assert!(neg);
        assert_eq!(d.decimal_point, 1);
        assert_eq!(d.num_digits, 3);
        assert_eq!(&d.digits[..3], &[3, 1, 4]);
    }

    // -----------------------------------------------------------------------
    // Trailing zeros stripped: "1.500" → digits [1, 5], decimal_point = 1
    // -----------------------------------------------------------------------

    #[test]
    fn trailing_zeros_stripped() {
        let (d, _) = parse("1.500").unwrap();
        assert_eq!(d.decimal_point, 1);
        assert_eq!(d.num_digits, 2);
        assert_eq!(&d.digits[..2], &[1, 5]);
    }

    // -----------------------------------------------------------------------
    // Post-decimal leading zeros: "0.005" → digits [5], decimal_point = -2
    // -----------------------------------------------------------------------

    #[test]
    fn post_decimal_leading_zeros() {
        let (d, _) = parse("0.005").unwrap();
        assert_eq!(d.decimal_point, -2);
        assert_eq!(d.num_digits, 1);
        assert_eq!(d.digits[0], 5);
    }

    // -----------------------------------------------------------------------
    // Pure zero: "0.0" → num_digits = 0 (all zeros stripped)
    // -----------------------------------------------------------------------

    #[test]
    fn zero_decimal() {
        let (d, _) = parse("0.0").unwrap();
        assert_eq!(d.num_digits, 0);
    }

    // -----------------------------------------------------------------------
    // Exponent positive: "1.5e2" → decimal_point = 1 + 2 = 3
    // -----------------------------------------------------------------------

    #[test]
    fn exponent_positive() {
        let (d, neg) = parse("1.5e2").unwrap();
        assert!(!neg);
        assert_eq!(d.decimal_point, 3);
        assert_eq!(d.num_digits, 2);
        assert_eq!(&d.digits[..2], &[1, 5]);
    }

    // -----------------------------------------------------------------------
    // Exponent negative: "1.5e-2" → decimal_point = 1 - 2 = -1
    // -----------------------------------------------------------------------

    #[test]
    fn exponent_negative() {
        let (d, _) = parse("1.5e-2").unwrap();
        assert_eq!(d.decimal_point, -1);
    }

    // -----------------------------------------------------------------------
    // Uppercase E: "2.0E3"
    // -----------------------------------------------------------------------

    #[test]
    fn uppercase_e() {
        let (d, _) = parse("2.0E3").unwrap();
        assert_eq!(d.decimal_point, 1 + 3); // decimal_point=1, exp=+3
    }

    // -----------------------------------------------------------------------
    // No post-decimal digits, exponent only: "1.e2" is malformed → Miss
    // -----------------------------------------------------------------------

    #[test]
    fn malformed_exponent_sign_only() {
        // "1.5e" — exponent marker but no digits
        assert!(parse("1.5e").is_none());
        assert!(parse("1.5e+").is_none());
        assert!(parse("1.5e-").is_none());
    }

    // -----------------------------------------------------------------------
    // Chunked: same result regardless of chunk boundaries
    // -----------------------------------------------------------------------

    #[test]
    fn chunked_matches_full() {
        let s = "3.14159";
        let full = parse(s).unwrap();
        for cs in 1..=s.len() {
            let chunked = parse_chunked(s, cs).unwrap_or_else(|| panic!("Miss at chunk_size={cs}"));
            assert_eq!(
                (chunked.0.num_digits, chunked.0.decimal_point, chunked.1),
                (full.0.num_digits, full.0.decimal_point, full.1),
                "chunk_size={cs}"
            );
        }
    }

    // -----------------------------------------------------------------------
    // Large exponent: "1.0e100"
    // -----------------------------------------------------------------------

    #[test]
    fn large_exponent() {
        let (d, _) = parse("1.0e100").unwrap();
        // decimal_point = 1 + 100 = 101
        assert_eq!(d.decimal_point, 101);
    }

    // -----------------------------------------------------------------------
    // parse_decimal_fast helpers
    // -----------------------------------------------------------------------

    fn parse_fast(s: &str) -> Option<(DecimalFast, bool)> {
        block_on(parse_decimal_fast(FakeAccess::new(s)))
            .ok()
            .and_then(|p| match p {
                Probe::Hit((_, d, neg)) => Some((d, neg)),
                Probe::Miss => None,
            })
    }

    fn parse_fast_chunked(s: &str, chunk_size: usize) -> Option<(DecimalFast, bool)> {
        block_on(parse_decimal_fast(FakeAccess::with_chunk_size(
            s, chunk_size,
        )))
        .ok()
        .and_then(|p| match p {
            Probe::Hit((_, d, neg)) => Some((d, neg)),
            Probe::Miss => None,
        })
    }

    #[test]
    fn fast_miss_on_integer() {
        assert!(parse_fast("42").is_none());
        assert!(parse_fast("-7").is_none());
    }

    #[test]
    fn fast_basic_decimal() {
        let (d, neg) = parse_fast("1.5").unwrap();
        assert!(!neg);
        assert_eq!(d.mantissa, 15);
        assert_eq!(d.exponent, -1);
        assert!(!d.many_digits);
    }

    #[test]
    fn fast_negative() {
        let (d, neg) = parse_fast("-3.14").unwrap();
        assert!(neg);
        assert_eq!(d.mantissa, 314);
        assert_eq!(d.exponent, -2);
    }

    #[test]
    fn fast_exponent_positive() {
        let (d, _) = parse_fast("1.5e2").unwrap();
        assert_eq!(d.mantissa, 15);
        assert_eq!(d.exponent, -1 + 2);
    }

    #[test]
    fn fast_exponent_negative() {
        let (d, _) = parse_fast("1.5e-2").unwrap();
        assert_eq!(d.mantissa, 15);
        assert_eq!(d.exponent, -1 - 2);
    }

    #[test]
    fn fast_no_dot_exponent_only() {
        let (d, _) = parse_fast("15e2").unwrap();
        assert_eq!(d.mantissa, 15);
        assert_eq!(d.exponent, 2);
        assert!(!d.many_digits);
    }

    #[test]
    fn fast_many_digits() {
        // 20 significant digits — many_digits must be true
        let (d, _) = parse_fast("1.2345678901234567890").unwrap();
        assert!(d.many_digits);
    }

    #[test]
    fn fast_malformed_exponent() {
        assert!(parse_fast("1.5e").is_none());
        assert!(parse_fast("1.5e+").is_none());
        assert!(parse_fast("1.5e-").is_none());
    }

    #[test]
    fn fast_chunked_matches_full() {
        let s = "3.14159e2";
        let full = parse_fast(s).unwrap();
        for cs in 1..=s.len() {
            let chunked =
                parse_fast_chunked(s, cs).unwrap_or_else(|| panic!("Miss at chunk_size={cs}"));
            assert_eq!(
                (
                    chunked.0.mantissa,
                    chunked.0.exponent,
                    chunked.0.many_digits,
                    chunked.1
                ),
                (full.0.mantissa, full.0.exponent, full.0.many_digits, full.1),
                "chunk_size={cs}"
            );
        }
    }
}
