//! Streaming JSON number parser for the owned family.
//!
//! Exposes four independent contestants that each consume a forked
//! [`NumberAccessOwned`] stream. The caller forks the access N ways and
//! races them via [`select_probe!`](strede::select_probe) — whichever
//! contestant matches the shape of the actual input wins; the others
//! return [`Probe::Miss`] without producing a value.
//!
//! - [`parse_int_pos`]   — bare digits, fits in `u64`.
//! - [`parse_int_neg`]   — leading `-`, fits in `i64`.
//! - [`parse_float_fast`] — Eisel-Lemire fast path; produces `(BiasedFp, negative)`.
//! - [`parse_float_slow`] — slow-mantissa fallback; produces `(BiasedFp, negative)`.
//!
//! Float contestants stop short of the final `f64` so the caller can
...
/// A custom 64-bit floating point intermediate, representing `m * 2^p_biased`.
///
/// `p_biased` is biased so it can be shifted directly into the exponent bits
/// of an `f64`.
#[derive(Debug, Copy, Clone, PartialEq, Eq, Default)]
pub(super) struct BiasedFp {
...
/// Streaming positive-integer contestant.
///
...
pub(super) async fn parse_int_pos<A: NumberAccessOwned>(
    ...
    let mut acc: u64 = 0;
    loop {
        let outcome = chunks.next_number_chunk(|s| fold_pos_digits(s, acc));
        match outcome.await? {
            Chunk::Data((next, Some(new_acc))) => {
                chunks = next;
                acc = new_acc;
            }
            Chunk::Data((_, None)) => return Ok(Probe::Miss),
            Chunk::Done(claim) => return Ok(Probe::Hit((claim, acc))),
        }
    }
}

/// Streaming negative-integer contestant.
///
...
pub(super) async fn parse_int_neg<A: NumberAccessOwned>(
    ...
    // Magnitude is accumulated as u64; `i64::MIN` has magnitude
    // `(i64::MAX as u64) + 1`, which fits in u64 but not i64 — handled at
    // finish time.
    let mut acc: u64 = loop {
        match chunks.next_number_chunk(consume_neg_sign).await? {
            Chunk::Data((next, NegFirst::SignThenDigits(a))) => {
                chunks = next;
                break a;
            }
            Chunk::Data((next, NegFirst::Empty)) => chunks = next,
            Chunk::Data((_, NegFirst::Miss)) => return Ok(Probe::Miss),
    ...
    loop {
        match chunks
            .next_number_chunk(|s| fold_pos_digits(s, acc))
            .await?
        {
            Chunk::Data((next, Some(new_acc))) => {
                chunks = next;
            ...
            Chunk::Data((_, None)) => return Ok(Probe::Miss),
            Chunk::Done(claim) => {
                let n = if acc == (i64::MAX as u64) + 1 {
                    i64::MIN
                } else if let Ok(x) = i64::try_from(acc) {
                    -x
                } else {
                    return Ok(Probe::Miss);
                };
                return Ok(Probe::Hit((claim, n)));
            }
        }
    }
}

/// First-chunk classification for `parse_int_neg`. Owned, so it can escape
/// the chunk closure's borrow.
enum NegFirst {
    /// Chunk started with `-`; remaining digits folded into `u64`.
    SignThenDigits(u64),
    /// Chunk was empty — caller pulls another.
    Empty,
    /// Leading byte was not `-`, or the post-sign digits were malformed /
    /// overflowed.
    Miss,
}

fn consume_neg_sign(s: &str) -> NegFirst {
    match s.as_bytes().first() {
        Some(&b'-') => match fold_pos_digits(&s[1..], 0) {
            Some(a) => NegFirst::SignThenDigits(a),
            None => NegFirst::Miss,
        },
        Some(_) => NegFirst::Miss,
        None => NegFirst::Empty,
    }
}

/// Fold a chunk of digit characters into the running u64 accumulator.
/// Returns `None` on non-digit byte or `u64` overflow.
fn fold_pos_digits(s: &str, mut acc: u64) -> Option<u64> {
    for &b in s.as_bytes() {
        if !b.is_ascii_digit() {
            return None;
        }
...
/// Streaming float contestant — Eisel-Lemire fast path.
///
/// Returns `Probe::Miss` when the fast path cannot guarantee a correctly
...
pub(super) async fn parse_float_fast<A: NumberAccessOwned>(
    ...
    // try_fast_path: pure float arithmetic for narrow exponent+mantissa range.
    if !num.many_digits
        && F64_MIN_EXP_FAST_PATH <= num.exponent
        && num.exponent <= F64_MAX_EXP_DISGUISED_FAST_PATH
        && num.mantissa <= F64_MAX_MANTISSA_FAST_PATH
    {
        let value = if num.exponent <= F64_MAX_EXP_FAST_PATH {
            let v = num.mantissa as f64;
            if num.exponent < 0 {
                v / f64_pow10((-num.exponent) as usize)
            } else {
                v * f64_pow10(num.exponent as usize)
            }
        } else {
            let shift = (num.exponent - F64_MAX_EXP_FAST_PATH) as usize;
            let Some(m) = num.mantissa.checked_mul(INT_POW10[shift]) else {
                return Ok(Probe::Miss);
            };
            if m > F64_MAX_MANTISSA_FAST_PATH {
                return Ok(Probe::Miss);
            }
            m as f64 * f64_pow10(F64_MAX_EXP_FAST_PATH as usize)
        };
        let fp = BiasedFp::from_f64(value);
        return Ok(Probe::Hit((claim, fp, negative)));
    }

    // Eisel-Lemire
    let mut fp = compute_float(num.exponent, num.mantissa);
    if num.many_digits && fp.p_biased >= 0 && fp != compute_float(num.exponent, num.mantissa + 1) {
        fp.p_biased = -1;
    }
    if fp.p_biased < 0 {
        return Ok(Probe::Miss);
    }
    Ok(Probe::Hit((claim, fp, negative)))
}

// ===========================================================================
// f64 fast-path constants
// ===========================================================================

const F64_MIN_EXP_FAST_PATH: i64 = -22;
const F64_MAX_EXP_FAST_PATH: i64 = 22;
const F64_MAX_EXP_DISGUISED_FAST_PATH: i64 = 37;
const F64_MAX_MANTISSA_FAST_PATH: u64 = 2_u64 << 52;
...
impl BiasedFp {
    fn from_f64(f: f64) -> Self {
        ...
        let p = (bits >> 52) as i32;
        BiasedFp { m, p_biased: p }
    }
}
...
/// High 64 bits of (5^i) << 128, for i in 0..=342 (covering 10^-342..10^308).
///
/// Table sourced from the Rust standard library's `dec2flt` implementation.
#[rustfmt::skip]
...
/// Eisel-Lemire: attempt to convert `(exponent, mantissa)` into a [`BiasedFp`].
///
/// Returns `p_biased = -1` when the result is ambiguous (caller should fall
/// back to the slow path).
fn compute_float(q: i64, mut w: u64) -> BiasedFp {
    // f64 constants.
    ...
    // Round-to-even safe exponent range: 5^q fits in 64 bits for |q| small enough.
    const MIN_EXPONENT_ROUND_TO_EVEN: i32 = -4;
    const MAX_EXPONENT_ROUND_TO_EVEN: i32 = 23;

    if w == 0 || q < SMALLEST_POWER_OF_TEN {
        return BiasedFp { m: 0, p_biased: 0 };
    }
    if q > LARGEST_POWER_OF_TEN {
        return BiasedFp {
            m: 0,
            p_biased: INFINITE_POWER,
        };
    }
    ...
    let mut mantissa = hi >> (upperbit + 64 - SIG_BITS - 3);
    let mut power2 = power(q as i32) + upperbit - lz as i32 - EXP_MIN + 1;
    ...
    if lo <= 1
        ...
        && mantissa & 0b11 == 0b01
        && (mantissa << (upperbit + 64 - SIG_BITS - 3)) == hi
    {
        mantissa &= !1_u64;
    }
...
/// Streaming float contestant — slow-path mantissa accumulator.
///
...
pub(super) async fn parse_float_slow<A: NumberAccessOwned>(
    ...
    let fp_zero = BiasedFp { m: 0, p_biased: 0 };
    let fp_inf = BiasedFp {
        m: 0,
        p_biased: INFINITE_POWER,
    };

    let (claim, mut d, negative) = hit!(parse_decimal_seq(chunks).await);

    // Short-circuit if the value can only be a literal 0 or infinity.
    if d.num_digits == 0 || d.decimal_point < -324 {
        return Ok(Probe::Hit((claim, fp_zero, negative)));
    } else if d.decimal_point >= 310 {
        return Ok(Probe::Hit((claim, fp_inf, negative)));
    }
    let mut exp2 = 0_i32;
    ...
    exp2 -= 1;
    while EXP_MIN > exp2 {
        let mut n = (EXP_MIN - exp2) as usize;
        if n > MAX_SHIFT {
            n = MAX_SHIFT;
        }
        d.right_shift(n);
    ...
    d.left_shift(SIG_BITS as usize + 1);
    let mut mantissa = d.round();
    if mantissa >= (1_u64 << (SIG_BITS + 1)) {
...
/// Convert a [`BiasedFp`] + sign into the final `f64`.
pub(super) fn biased_fp_to_float(fp: BiasedFp, negative: bool) -> f64 {
    // f64: 52 stored mantissa bits (the leading 1 is implicit).
    const SIG_BITS: u32 = f64::MANTISSA_DIGITS - 1;
    let mut word = fp.m;
    word |= (fp.p_biased as u64) << SIG_BITS;
